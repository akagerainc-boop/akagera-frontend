...
export default function GoogleAuthButton({ onDone, label = 'Continue with Google' }) {
  ...
  return (
    <div>
      <button className="btn btn--secondary btn--block" onClick={handle} disabled={loading}>
        {loading ? <IosSpinner size="sm" /> : (
          <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden>
            ...
            <path fill="#4CAF50" d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.2C29.3 35 26.8 36 24 36c-5.3 0-9.7-3.1-11.3-7.5l-6.5 5C9.6 39.6 16.2 44 24 44z" />
      ...
      {error && <p className="mt-1" style={{ color: 'var(--err)', fontSize: '.85rem' }}>{error}</p>}
    </div>
  );
}
